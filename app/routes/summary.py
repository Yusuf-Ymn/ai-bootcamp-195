...
def process_daily_summary(request: SummaryRequest, db: Session = Depends(get_db)):
    try:
        ...
        suggestions = suggest_lifestyle_tips(request.metrics.model_dump())
        ...
        db.rollback()
        print(f"Daily summary hatası: {e}")
        raise HTTPException(status_code=500, detail=f"Günlük özeti işlenirken hata oluştu: {str(e)}")
